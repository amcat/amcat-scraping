...
class NewsdeskScraper(SeleniumLoginMixin, SeleniumMixin, DeduplicatingUnitScraper):
    ...
    def get_deduplicate_units(self) -> Iterable[NewsdeskUnit]:
        ...
        self.wait('//span[text() = "{}"]'.format("7 dagen"), by=By.XPATH).click()
        self.wait('//span[text() = "{}"]'.format("7 dagen"), by=By.XPATH).click()

        #Set online news media
        self.wait(".search-area__filters-trigger").click()
        self.wait('//span[text() = "{}"]'.format("Bronnenlijsten"), by=By.XPATH).click()
        self.wait('//i[@aria-label=\'Opnemen "Online BZK"\']', by=By.XPATH).click()

        # Search!
        self.wait(".rich-search-box__content").click()
        self.wait(".rich-search-box__content").send_keys('Ollongren OR "Raymond Knops" OR "staatssecretaris Knops" OR (woordvoerder* NEAR/5 BZK OR "binnenlandse zaken") OR ((Koninkrijksrelaties OR Knops OR "binnenlandse zaken") AND ("Sint-Maarten" OR bes-eiland* OR Bonaire OR "Sint Eustatius" OR Saba OR "Caribisch Nederland" OR curacao OR Aruba)) OR AIVD* OR "Inlichtingen- en Veiligheidsdienst*" OR inlichtingendienst OR veiligheidsdienst OR Rijksvastgoed* OR rijksgebouw* OR "renovatie Binnenhof" OR "verbouwing Binnenhof" OR "huis voor klokkenluiders" OR "Rijksdienst voor Identiteitsgegevens" OR VNG OR IPO OR Aedes OR woonbond* OR "Bam Wonen" OR "Vereniging eigen huis" OR "Bouwend Nederland" OR "Basisregistratie personen" OR brp* OR "digitale overheid" OR digid OR "correctief referendum" OR "raadgevend referendum" OR referendumwet OR kieswet OR kiesraad OR omgevingswet OR NOVI OR "grensoverschrijdende samenwerking" OR "Versterk* Groningen" OR "Wet Normering Topinkomens" OR "WIV" OR sleepwet OR sleepnet OR aftapwet OR afluisterwet OR "Kwaliteitsborging bouw" OR woningwet OR "Nationale Hypotheek Garantie" OR "NHG" OR zondagswet OR rotterdamwet OR ((Knops OR Ollongren OR (minister* NEAR/5 "Binnenlandse zaken")) AND (Cyberdreiging* OR "agenda stad" OR burgemeester* OR decentralisatie* OR woningmarkt OR woningcorporati* OR corporati* OR huurder* OR gasketel OR cv-ketel OR (Aardbeving* NEAR/25 Groning*) OR aardgasvrij* OR ABP OR pensioenstelsel OR pensioenfonds* OR grondwet*)) NOT pools* NOT Caracao NOT "Wit-Rusland" NOT oostenrijk* NOT fran*')
    ...
    def scrape_unit_meta(self, article_element):
     ...
     #   print(f"dit is html{article_html}")
        ...
        print(date_text)
        date = date_text.split("Gepubliceerd op:")
        pub_date = date[-1]
